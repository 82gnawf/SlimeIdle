...
public class GlobalGreenSeller : MonoBehaviour {
    ...
    void Update () {
        ...
        SellerStat.GetComponent<Text>().text = "Green Seller Lvl." + SellerLevel + " (" + SellerSPS + " SPS)";
        displaySellerValue = Convert.ToInt32(SellerValue);

        if (SellerLevel == 0) { //lvl statements
            fakeText.GetComponent<Text>().text = "Hire Green Seller - $" + displaySellerValue; //button display
            realText.GetComponent<Text>().text = "Hire Green Seller - $" + displaySellerValue;
        }
        else {
            fakeText.GetComponent<Text>().text = "Upgrade Green Seller - $" + displaySellerValue;
            realText.GetComponent<Text>().text = "Upgrade Green Seller - $" + displaySellerValue;
        }
